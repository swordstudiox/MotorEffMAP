...
- 电机效率 MAP 图。
- 系统效率 MAP 图。
- MCU / 电机 / 系统转速-功率-效率 MAP 图。
- MCU / 电机 / 系统转速-扭矩-损耗 MAP 图。
- 外特性曲线图。
- 效率区域占比 Excel。
- 效率区域占比曲线图。
...
| `MotorEffMAP_GUI.py` | 旧导入兼容入口，重新导出 `motor_eff_map.gui` 中的 GUI 类。 |
| `MotorEffMAP_Logic.py` | 旧导入兼容入口，重新导出 `motor_eff_map.logic.MotorEffLogic`。 |
| `motor_eff_map/gui/main_window.py` | GUI 主体布局、配置刷新、轻量协调辅助和旧信号连接入口。 |
| `motor_eff_map/gui/config_schema.py` | GUI 配置标签、输出开关、效率输出定义、默认配置和图形布局常量。 |
| `motor_eff_map/gui/widgets.py` | 日志控件、固定比例画布容器、页脚签名控件。 |
| `motor_eff_map/gui/config_editor.py` | 配置页相关的 INI 读取、默认值补齐、控件创建和写回。 |
| `motor_eff_map/gui/plot_helpers.py` | figure 布局、等高线级别、masked contour 网格、坐标刻度和图像缓存。 |
| `motor_eff_map/gui/output_naming.py` | 输出文件名清洗和输出 stem 生成。 |
...
| `motor_eff_map/logic/motor_eff_logic.py` | 数据读取、列映射、清洗、归一化、包络线、插值网格、损耗和效率区域占比计算。 |
| `motor_eff_map/logic/config_values.py` | 逻辑层配置文本、正数、非负数、起始功率和等高线步长解析。 |
| `motor_eff_map/logic/interpolation.py` | 插值点有效性校验和起始转速/扭矩/功率裁剪辅助。 |
...
```mermaid
flowchart LR
    U["用户"] --> RUN["run.py<br/>应用入口"]
    RUN --> GUI_COMPAT["MotorEffMAP_GUI.py<br/>旧导入兼容层"]
    GUI_COMPAT --> GUI["motor_eff_map/gui/main_window.py<br/>PySide6 主窗口布局"]
    GUI --> SCHEMA["config_schema.py<br/>配置元数据"]
    GUI --> CTRL["processing_controller.py<br/>批处理编排"]
    CTRL --> WORKER["batch_worker.py<br/>QThread 后台批处理"]
    GUI --> PLOTTERS["plotters/<br/>按图类型绘图"]
    GUI --> INI["MotorEffMAP.ini<br/>配置解析与保存"]
    GUI --> LOGIC["motor_eff_map/logic<br/>数据处理核心"]
    LEGACY["外部旧脚本 / 测试"] --> LOGIC_COMPAT["MotorEffMAP_Logic.py<br/>旧导入兼容层"]
    LOGIC_COMPAT --> LOGIC
    PLOTTERS --> PLOT["matplotlib<br/>MAP 图与占比图"]
    CTRL --> LOGIC
    WORKER --> LOGIC
    WORKER --> PLOTTERS
    LOGIC --> XLSX["Excel 工作簿<br/>多文件 / 多 Sheet"]
    LOGIC --> NUM["numpy / scipy<br/>网格、插值、区域统计"]
    PLOTTERS --> OUT["输出文件<br/>PNG / XLSX / Log"]
```

当前 `MotorEffMAP_GUI.py` 和 `MotorEffMAP_Logic.py` 是兼容入口，真实实现已经拆分到 `motor_eff_map/gui/` 与 `motor_eff_map/logic/`。旧入口仍保留，是为了兼容现有脚本、测试和打包流程；新增功能优先放入包内对应模块。
...
处理流程由 `MainWindow.run_process_all()` 启动。批量保存时，GUI 线程只负责读取配置快照、禁用相关按钮、创建 `BatchWorker` 并接收进度/完成/失败信号；Excel 读取、列映射、归一化、插值、PNG/XLSX 导出都在 `QThread` 后台执行。手动点击单个视图按钮仍走 GUI 的交互式绘图路径，用于查看当前数据。

后台批处理使用 `BatchExportContext` 复用现有绘图 mixin，但使用独立 `matplotlib.figure.Figure` 和 Agg canvas，不复用界面上的 QtAgg canvas，也不直接操作 `QMessageBox`、`QListWidget` 或进度条。这样单个 sheet 计算或保存耗时较长时，Qt 主事件循环仍能响应窗口消息。
...
| `BatchWorker` | 后台线程 | 按配置快照处理所有 file + sheet，并通过 signal 汇报进度和错误。 |
| `BatchExportContext` | 后台线程 | 持有独立 `MotorEffLogic`、`Figure` 和 Agg canvas，负责批量导出 PNG/XLSX。 |
...
| `SpeedPowerMAP` | 是否输出转速-功率-效率 MAP。 | `1` |
| `LossMAP` | 是否输出转速-扭矩-损耗 MAP。 | `1` |
| `ExternalCharacteristicPlot` | 是否输出外特性曲线图。 | `1` |
...
| `StartSpeed` | 起始转速。低于该值的区域屏蔽。 | `0` |
| `StartTorque` | 转速-扭矩效率 MAP、损耗 MAP 和占比分母的起始扭矩。 | `0` |
...
坐标刻度按物理量拆分：转速-扭矩效率 MAP 和损耗 MAP 使用 `xstepSpeed` / `ystepTorque`，转速-功率-效率 MAP 使用 `xstepSpeed` / `ystepPower`，外特性图横轴使用 `xstepSpeed`。
...
转速-扭矩效率 MAP 使用 `StartSpeed` 和 `StartTorque` 生成截止掩码：

```python
cutoff_mask = (XI < StartSpeed) | (YI < StartTorque)
```

被截止的效率和功率会设为 NaN。转速-功率-效率 MAP 会在此基础上把扭矩坐标换算为功率坐标，并额外使用 `StartPower` 裁剪低功率区域；`StartPower` 留空时由 `StartSpeed * StartTorque / 9550` 换算得到。
...
`StartSpeed` 和 `StartTorque` 同时影响转速-扭矩效率 MAP、损耗 MAP 的图形显示区域和效率占比分母。低于起始转速或起始扭矩的点会被截止：
...
被 `cutoff_mask` 命中的区域会把 `ZI_Eff` 和 `ZI_Power` 设为 NaN，同时不再计入 `mask_valid_geo`。因此用户把 `StartSpeed=50`、`StartTorque=5` 时，转速-扭矩效率 MAP、损耗 MAP 和占比分母都会从这个坐标开始。

转速-功率-效率 MAP 的横轴仍使用 `StartSpeed`，纵轴使用 `StartPower`。如果 `StartPower` 留空，程序按 `StartSpeed * StartTorque / 9550` 自动换算；如果填写了 `StartPower`，则优先使用填写值。这使功率图可以独立控制低功率区域，而不强制复用扭矩图的纵轴下限。

图 11 图解路线：

1. 生成转速-扭矩截止掩码：`cutoff_mask = (XI < StartSpeed) | (YI < StartTorque)`，低转速或低扭矩任一条件命中都截止。
2. 转速-功率图额外生成低功率截止：`YI_Power < StartPower` 的区域不显示，也不计入对应几何区域。
3. 同步调整分母：`mask_valid_geo = 包络线内 & ~cutoff_mask`，占比只按保留区域统计。

重点：`StartTorque` 面向扭矩坐标图，`StartPower` 面向功率坐标图；二者不能互相替代。
...
| 坐标起点 | 转速-扭矩效率 MAP 和损耗 MAP 从 `StartSpeed` / `StartTorque` 开始；转速-功率-效率 MAP 从 `StartSpeed` / `StartPower` 开始。 |
| 保存 DPI | `200`。 |
...
除普通转速-扭矩效率 MAP 外，当前 GUI 还按图类型拆分了三个专用绘图入口：

| 图类型 | 绘图入口 | 坐标与配置口径 |
| --- | --- | --- |
| 转速-功率-效率 MAP | `show_speed_power_efficiency_plot()` | X 轴为转速，Y 轴为功率；刻度使用 `xstepSpeed` / `ystepPower`，显示起点使用 `StartSpeed` / `StartPower`。 |
| 转速-扭矩-损耗 MAP | `show_loss_map_plot()` | X 轴为转速，Y 轴为扭矩；刻度使用 `xstepSpeed` / `ystepTorque`，显示起点使用 `StartSpeed` / `StartTorque`。 |
| 外特性曲线图 | `show_external_characteristics_plot()` | X 轴为转速，左 Y 轴为扭矩，右 Y 轴为功率；横轴刻度使用 `xstepSpeed`，双纵轴使用专用右边距。 |

损耗 MAP 的损耗值来自 `P_Motor(kW) * 1000 * (100 / 效率% - 1)`，效率为 0、空值或大于等于 100 时损耗保持 NaN。转速-功率-效率 MAP 由转速-扭矩效率网格换算功率坐标，不使用最近邻补齐原效率 MAP 中没有可靠值的区域。
...
2. 输出类型：效率图追加 `MCU/Motor/System EfficiencyMAP`，转速-功率图追加 `控制器/电机/系统 SpeedPowerEfficiencyMAP`，损耗图追加 `控制器/电机/系统 LossMAP`，外特性图追加 `外特性曲线`，占比结果追加 `效率占比.xlsx/png`。
3. 文件名清理：保留中文、字母、数字、下划线、点和短横线，其它字符替换为下划线并折叠连续下划线。

重点：README 和 docs 正文中的说明性示例应使用匿名名称，例如“示例数据”和“车型A”；仓库 `example/` 中实际存在的示例资产可以按真实文件名引用，但不得混入未公开的项目代号或电压平台。
...
```text
python -m PyInstaller --noconfirm --onedir --windowed --name MotorEffMAP --clean --hidden-import=openpyxl --hidden-import=xlrd run.py
```
...
| 修改类型 | 必查文件 | 必跑验证 |
| --- | --- | --- |
| 新增或删除配置项 | `MotorEffMAP.ini`、`motor_eff_map/gui/config_editor.py`、README、本文档 | 配置页保存、`git diff --check` |
| 修改数据清洗或插值 | `motor_eff_map/logic/`、测试、本文档第 8 章 | 单元测试、目标 Excel 回归样本 |
| 修改图形版式 | `motor_eff_map/gui/main_window.py`、`motor_eff_map/gui/plot_helpers.py`、本文档第 9 章 | GUI 显示比例、保存 PNG 比例 |
